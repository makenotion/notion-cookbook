...
description: Review Notion App workflows for trigger typing and retry safety.
user-invocable: true
...
1. A workflow is not a direct file or does not default-export
   `createWorkflow`.
...
5. A step result is not JSON-serializable or later code needs an in-memory
   mutation.
