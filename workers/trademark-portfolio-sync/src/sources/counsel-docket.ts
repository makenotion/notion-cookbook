...
async function fetchInboxFiles(pageId: string): Promise<InboxFile[]> {
  ...
  // 10 pages = 1,000 blocks, far beyond any real inbox — the cap only
  // guards against a cursor loop.
  for (let page = 0; page < 10; page++) {
    ...
    if (!body.has_more || !body.next_cursor) return files
...
export function parsePropertiesReport(
  ...
  for (const row of grid.slice(1)) {
    ...
    const app = digits(d["Application #"])
    ...
    entries.push({
      ...
      registrationNumber: digits(d["Registration #"]),
