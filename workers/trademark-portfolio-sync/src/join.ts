...
export type PortfolioRow = {
  key: string // US serial | ST13 | DKT-{office}-{number}
...
export type StatusBucket =
  "Registered" | "Pending" | "Abandoned" | "Cancelled" | "Expired"
...
function makeDocketCtx(d: CounselDocketData): DocketCtx {
  ...
  // office+number keys (and bare-number fallback for Madrid IRs) for every
  // lapse-instructed entry, so registry rows can carry the flag too.
...
// office + serial/registration digits; earliest due date wins when
// several actions are docketed (e.g. an SOU plus its backup extension).
// The digit path REQUIRES the action to carry an office: registration and
// IR numbers collide across offices after zero-stripping.
function docketDeadlineFor(
  ...
  const digits = numbers.map((n) => (n ? stripZeros(n) : "")).filter(Boolean)
  const hits = docket.actions
    .filter((a) => {
      ...
      return digits.includes(stripZeros(a.number))
    })
...
function lapseInstructedFor(
  ...
  return numbers.some((n) => {
    const d = n ? stripZeros(n) : ""
    return (
      d !== "" &&
      (docket.lapseKeys.has(`${office}-${d}`) ||
        (office === "WO" && docket.lapseKeys.has(d)))
    )
  })
}
...
function buildUsRow(
  ...
  if (c.statusDate) properties["Status Date"] = Builder.date(c.statusDate)
  if (c.filingDate) properties["Filed"] = Builder.date(c.filingDate)
  if (c.publishedDate) properties["Published"] = Builder.date(c.publishedDate)
  if (c.registrationDate)
    properties["Registered"] = Builder.date(c.registrationDate)
  if (c.registrationNumber && c.register)
    properties["Register"] = Builder.select(c.register)
  if (basisName) properties["Basis"] = Builder.select(basisName)
...
function buildForeignRow(
  ...
  const st = m.status ?? ""
  const bucket: StatusBucket = /registered|protect/i.test(st)
    ? "Registered"
    : /expir/i.test(st)
      ? "Expired"
      : /end|withdraw|refus|abandon|cancel|dead/i.test(st)
        ? "Abandoned"
        : "Pending"
  ...
  if (m.statusDate) properties["Status Date"] = Builder.date(m.statusDate)
  if (m.applicationDate) properties["Filed"] = Builder.date(m.applicationDate)
  if (m.registrationDate)
    properties["Registered"] = Builder.date(m.registrationDate)
  if (config.sources.counselDocket) {
    properties["Lapse Instructed"] = Builder.checkbox(
...
// Keyed DKT-{office}-{number}; status is counsel's, mapped to the buckets,
// with the raw text kept in Office Status with a provenance suffix.
function buildDocketOnlyRow(
  ...
  const number = e.registrationNumber || e.applicationNumber
  ...
  const key = `DKT-${e.office}-${stripZeros(number) || e.mark.replace(/\W+/g, "")}`
  const properties: Record<string, unknown> = {
  ...
  if (e.filedDate) properties["Filed"] = Builder.date(e.filedDate)
  if (e.registrationDate)
    properties["Registered"] = Builder.date(e.registrationDate)
  if (config.sources.counselDocket)
...
export async function buildPortfolioRows(opts: {
  ...
  let docket: DocketCtx = EMPTY_DOCKET
  const inboxPage = config.sources.counselDocket ? docketInboxPageId() : null
  if (config.sources.counselDocket && !inboxPage) {
    console.warn(
      "[join] sources.counselDocket is on but DOCKET_INBOX_PAGE_ID is not set — skipping docket ingestion"
    )
  }
  if (inboxPage) {
    const counsel = await runner.run<CounselDocketData>(
      "counselDocket",
      () =>
        fetchCounselDocket(
          inboxPage,
  ...
  const applyOverlay = (m: ForeignMark, o: OfficialOverlay) => {
    m.status = o.status ?? m.status
    m.statusDate = o.statusDate ?? m.statusDate
    m.applicationDate = o.applicationDate ?? m.applicationDate
    m.registrationDate = o.registrationDate ?? m.registrationDate
    m.expirationDate = o.renewalDue ?? m.expirationDate
  }
  if (config.sources.ipAustralia) {
    try {
      const nums = Object.values(foreign)
        ...
        .sort()
      const overlays = await runner.run<Record<string, OfficialOverlay>>(
        "ipAustralia",
      ...
      for (const m of Object.values(foreign)) {
        if (
          m.office === "AU" &&
          m.applicationNumber &&
          overlays[m.applicationNumber]
        )
          applyOverlay(m, overlays[m.applicationNumber])
      }
    } catch (err) {
      console.warn(
  ...
  let spendBySerial: Record<string, SpendInfo> = {}
  if (config.sources.spend) {
    try {
      spendBySerial = await runner.run<Record<string, SpendInfo>>("spend", () =>
        spendAdapter.lookup(
  ...
  // 5. Rows. Deterministic order (the backfill pages over this list).
  ...
  // same office and aren't designations of a known Madrid IR (those live
  // on the WO row's Jurisdiction).
  const known = new Set<string>()
  const irDigits = new Set<string>()
  for (const c of Object.values(usCases)) {
    for (const n of [c.serial, c.registrationNumber]) {
      if (n) known.add(`US-${stripZeros(n)}`)
    }
  }
  for (const m of Object.values(foreign)) {
    for (const n of [m.applicationNumber, m.registrationNumber]) {
      if (n) known.add(`${m.office}-${stripZeros(n)}`)
    }
    if (m.office === "WO" && m.registrationNumber)
      irDigits.add(stripZeros(m.registrationNumber))
  }
  let docketOnly = 0
  for (const e of docket.entries) {
    ...
    if (nums.some((n) => irDigits.has(n))) continue // IR designation
  ...
  // Last Sync is added AFTER the fingerprint basis was captured (the
  // builders return fingerprintBasis === properties-at-build-time), so it
  // advances only on rows that re-emit for a real change.
  for (const r of rows) {
    r.fingerprintBasis = { ...(r.properties as Record<string, unknown>) }
    r.properties["Last Sync"] = Builder.date(today)
  }
