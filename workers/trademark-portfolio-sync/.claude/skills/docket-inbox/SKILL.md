...
- **Deadline overrides** (`docketDeadlineFor`): an action matches a row by
  ...
  serial/registration digits — the digit path REQUIRES the action to carry
  an office, because numbers collide across offices after zero-stripping.
  Earliest due date wins (e.g. an SOU plus its backup extension), and
...
- **Lapse Instructed** comes from `LAPSE_STATUSES` (`ALLOW TO LAPSE`,
  `RENUNCIATION`) keyed by office + digits, with bare digits accepted for WO
  so Madrid IR designations inherit the flag.
- **Docket-only rows:** entries matching no registry row in the same office
  (and not a designation of a known IR) become `DKT-{office}-{number}` rows —
  counsel's status mapped to the buckets, the raw text kept in Office Status
