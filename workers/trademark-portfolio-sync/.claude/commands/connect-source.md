...
`source-adapter` skill — it has the adapter contracts and the pacer /
...
counsel, verification). The `docket-inbox` skill documents the pipeline and
...
"no overlay yet" (a warning, not a failure), and the **EUIPO** health row
probes a real data call precisely so it stays red until approval lands. A red
EUIPO row with everything else green means "still waiting on EUIPO", not a
bug.
...
2. Implement `lookup(keys)` in `src/sources/spend.example.ts`: `keys` are
   ...
   matter, read the `sync-engine` skill on budgeting — chunk across cycles.
