...
everything here for you (`/setup` is the guided entry point); deeper design
notes — the rules that keep the engine from breaking — live in
[`AGENTS.md`](AGENTS.md) and `.claude/skills/`.
...
state has a ~200KB _run-input_ limit below the 256KB _save_ cap, so snapshots
are gzipped and the adapters store already-normalized records, not raw API
payloads. Change detection is fingerprint-based: bump `DERIVATION_VERSION`
...
- **Another office or register** — a new adapter in `src/sources/` against
  the contracts in `src/sources/types.ts`; see the `source-adapter` skill.
...
by default; the `document-retrieval` skill documents the details.
...
AGENTS.md                # context for your AI assistant (CLAUDE.md is a copy)
.claude/                 # /setup command + skills (engine, sources, docket, documents)
```
