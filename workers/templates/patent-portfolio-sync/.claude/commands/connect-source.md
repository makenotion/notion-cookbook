...
Help the user wire a source into the portfolio. First read
`domain-guides/source-adapter/SKILL.md` — it has the `PatentRecord` contract
and the pacer / projection rules. Ask which kind of source this is, then:
...
1. Auth → `.env`. 2. Implement `lookup(keys)` in `spend.example.ts` returning
   ...
   `domain-guides/sync-engine/SKILL.md` on resolution budgeting — chunk across
   cycles, don't fetch everything in one execute. 3. Set
   `config.sources.spend = true`. 4. Verify.
