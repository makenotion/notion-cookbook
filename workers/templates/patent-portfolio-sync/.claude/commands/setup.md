...
Ask for the exact applicant/assignee name(s) as registered with patent offices
(e.g. "Acme Corporation"). This drives USPTO + EPO discovery. Write them to
`PORTFOLIO_APPLICANTS` in `.env` as a JSON string array so names such as
`Acme, Inc.` retain their commas. Mention they can list several and that the
base USPTO query covers first-named applicants in public Patent File Wrapper
data filed after January 1, 2001; it is not a complete ownership search.
...
Tell them the baseline tracks identity, status, grants, and baseline patent
term estimates. Explain that **Est. Expiry is not a docketing deadline** and
omits PTA, PTE, terminal disclaimers, reissue adjustments, SPCs, and national
extensions. Then offer the advanced set as an opt-in and ask
...
- US term & prosecution metadata (PTA, Track One, art unit, and terminal
  disclaimer; no calculated legal deadlines)
- EP orphan audit (EP filings missing from your docket) — needs docketing
- EP register detail (designated states, EPO application renewal payments,
  and X/Y citations; no post-grant national renewal or legal status)

If they pick any, run `/add-advanced-enrichment` with their selections.

If they need non-US/EP grant or current legal-status data, or a `Next Deadline`,
route them to an authoritative docketing source or jurisdiction-specific
official-register adapter through `/connect-source`; do not offer inferred
kind-code, legal-event-influence, or date-arithmetic fields.
