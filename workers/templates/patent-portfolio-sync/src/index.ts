...
// invoked, so they add no background sync load; `attach` uses the
// NOTION_API_TOKEN injected for Custom Agent calls (set it explicitly only for
// local execution). See domain-guides/document-retrieval/SKILL.md. Remove this
...
const enabledSourceKeys = (): string[] => [
  ...(config.sources.uspto ? ["uspto"] : []),
  ...(config.sources.epo ? ["epo"] : []),
  ...(config.sources.docketing ? ["docketing"] : []),
  ...(config.sources.spend ? ["spend"] : []),
]

// Compact marker for assembly-affecting source toggles. Read at execute time
// so local tests and deploy-time configuration changes follow the same path.
const sourceConfigSignature = (): string =>
  `u${Number(config.sources.uspto)}e${Number(config.sources.epo)}d${Number(config.sources.docketing)}s${Number(config.sources.spend)}`

function snapshotsCoverEnabledSources(snapshots: SourceSnapshots): boolean {
  return enabledSourceKeys().every((key) =>
...
function selectRows(
  ...
  return keys.map((key) => {
    ...
    if (!row)
      throw new Error(
        `[${label}] frozen key "${key}" is absent from its snapshot`
      )
    return row
  })
}
...
function parseBackfillState(state: unknown): BackfillState | undefined {
  ...
  if (
    ...
    (state.configSignature !== undefined &&
  ...
  if (
    state.configSignature !== undefined &&
    state.configSignature !== sourceConfigSignature()
  )
    throw new Error(
...
worker.sync("portfolioBackfill", {
  ...
  execute: async (rawState: BackfillState | undefined) => {
    ...
    if (state) {
      // Pure emission phase: reconstruct exclusively from the frozen source
      // snapshots. No office API call can move the page boundary mid-cycle.
      ...
      if (!snapshotsCoverEnabledSources(snapshots))
        throw new Error(
          "[portfolioBackfill] frozen snapshots do not cover the currently enabled sources; restart the replace cycle"
        )
      const assembled = assemblePortfolioRows({
        snapshots,
        nowIso: cycleNowIso,
      })
      rows = selectRows(
      ...
      // Strict acquire phase. If this fetch or the state-size guard fails,
      // execute throws before the first replace-mode change is returned.
...
worker.sync("portfolioDelta", {
  ...
  execute: async (rawState: DeltaState | undefined) => {
    ...
    if (parsed.kind === "emit") {
      // A frozen cycle is entirely local. This makes page-2 outages
      // irrelevant and prevents any fresh row from being rolled back to LKG.
      ...
      if (!snapshotsCoverEnabledSources(snapshots))
        throw new Error(
          "[portfolioDelta] frozen snapshots do not cover the currently enabled sources; restart the incremental cycle"
        )
      const assembled = assemblePortfolioRows({
        snapshots,
      ...
      const pendingRows = selectRows(assembled, pendingKeys, "portfolioDelta")
    ...
    if (parsed.kind === "steady") {
      ...
      const configMatches =
        parsed.state.configSignature === undefined ||
        parsed.state.configSignature === sourceConfigSignature()
      const snapshotsComplete = snapshotsCoverEnabledSources(prevSnapshots)
      if (
        ...
        console.warn(
          "[portfolioDelta] enabled sources changed or prior snapshots are incomplete; discarding the fingerprint baseline and forcing a full re-emit"
        )
      }
      // A version mismatch deliberately leaves base empty and forces a full
      // one-time re-emit under the new derivation rules.
...
const ALL_HEALTH_ENDPOINT_NAMES = ["USPTO", "EPO OPS"] as const
...
function normalizedHealthTimestamp(value: unknown): string | null {
  if (typeof value !== "string") return null
  // Migrate date-only state written by earlier worker versions so a failed
  // first probe after upgrade cannot feed an invalid value to dateTime().
  if (/^\d{4}-\d{2}-\d{2}$/.test(value)) {
    return canonicalUtcTimestamp(`${value}T00:00:00.000Z`)
  }
  ...
  execute: async (state: HealthState | undefined) => {
    ...
    const results = await Promise.all(
      HEALTH_ENDPOINTS.map(async (ep) => {
        ...
        try {
          ...
          error =
            rawError.length > MAX_PERSISTED_ERROR_CHARS
              ? `${rawError.slice(0, MAX_PERSISTED_ERROR_CHARS - 3)}...`
              : rawError
        }
        const rawPrevious = prev[ep.name]
        const p: HealthEntry = {
          lastSuccessAt: normalizedHealthTimestamp(rawPrevious?.lastSuccessAt),
          downSince: normalizedHealthTimestamp(rawPrevious?.downSince),
          consecutiveFailures:
            Number.isSafeInteger(rawPrevious?.consecutiveFailures) &&
            (rawPrevious?.consecutiveFailures ?? -1) >= 0
              ? rawPrevious!.consecutiveFailures
              : 0,
        }
    ...
    const changes = results.map(({ name, ok, error, entry }) => {
      const props: Record<string, unknown> = {
        ...
        "Last Checked": Builder.dateTime(now),
        "Last Success": entry.lastSuccessAt
          ? Builder.dateTime(entry.lastSuccessAt)
          : [],
    ...
    // Incremental syncs do not remove rows merely because they stop being
    // emitted. Delete known disabled endpoints explicitly so toggling an office
    // off cannot leave a stale (and possibly Down) health-dashboard row.
    const enabledNames = new Set(
      HEALTH_ENDPOINTS.map((endpoint) => endpoint.name)
    )
    const disabledChanges = ALL_HEALTH_ENDPOINT_NAMES.filter(
      (name) => !enabledNames.has(name)
    ).map((name) => ({ type: "delete" as const, key: name }))

    return {
      changes: [...changes, ...disabledChanges] as never,
      hasMore: false,
      nextState,
    }
  },
