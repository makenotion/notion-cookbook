...
`workers/templates/patent-portfolio-sync/` folder. You need a copy of it on your
...
and open the workers/templates/patent-portfolio-sync folder"_ — and it'll handle
the rest.
...
- **Download a ZIP (no Git needed):** on the cookbook's GitHub page
  ...
  `workers/templates/patent-portfolio-sync/`.
...
`workers/templates/patent-portfolio-sync/` inside the cookbook you just
downloaded.
...
- **Your company's applicant/assignee name** — exactly as it appears on your
  ...
  >
  > The base USPTO discovery query matches the first-named applicant in public
  > Patent File Wrapper data for applications filed after January 1, 2001. It
  > is a portfolio starter, not a complete ownership search; reconcile it
  > against your authoritative docket.
...
database is created too — it flags an outage at an enabled USPTO or EPO
endpoint; optional docketing and spend adapters do not get probe rows.)
...
> (re-check it on Google Patents as above). A company listed only as a later US
> co-applicant is also outside the base query and should be reconciled through
> your docket or a customized source.
>
...
- **Families** — one row per family, with its applications nested underneath.
  ...
  A→Z. Show: Title, # Apps, # Grants, Filing Date (and Total Spend if you've
  connected spend). Click a family's ▸ to expand its apps. Status and Est.
  Expiry belong to the nested application rows, so family rows leave them blank.
...
status, dates, grant/publication numbers, baseline expiry estimates, family
counts, and more. The base **Est. Expiry** excludes adjustments such as PTA,
PTE, terminal disclaimers, reissue term, SPCs, and national extensions; never
use it as a legal deadline without checking the authoritative docket. It stays
blank if a grant or term-bearing continuity filing date is unavailable rather
than guessing from a later date. This is intentionally a starting point, not a
finished layout. Shape it to
...
### Portfolio term review

Surfaces patents whose baseline statutory term estimate is approaching. This is
a planning signal, not a renewal or deadline-docketing system.

- **Trigger:** scheduled, weekly.
- **Instructions:** _"In the Patent Portfolio database, find every row where
  **Est. Expiry** is within the next 6 months, excluding rows whose **Status**
  is abandoned or expired. Post a planning digest grouped by **Jurisdiction**,
  each line showing Title, App. No. / Patent #, and the estimated date. Label
  every date **baseline estimate — verify against the authoritative docket**.
  If nothing is approaching, say so."_
...
it under a Notion page. They run on demand (no extra sync load); Custom Agent
calls inject the Notion token used by `attach`, while local execution must set
`NOTION_API_TOKEN`. Ask your assistant to "list/attach prosecution documents,"
and see
[`domain-guides/document-retrieval/SKILL.md`](domain-guides/document-retrieval/SKILL.md)
for details.
...
live in [`DEVELOPMENT.md`](DEVELOPMENT.md) and
[`domain-guides/`](domain-guides/). `AGENTS.md`, `CLAUDE.md`, and
`.claude/skills` expose the cookbook's generated canonical Worker guidance.
