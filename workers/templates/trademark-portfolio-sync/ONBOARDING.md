...
- The **`ntn` command-line tool** — Notion's CLI that connects to your
  ...
  _In short, you don't have to run any of these yourself — the guided setup
  in Step 3 checks for Node and `ntn` and installs anything missing.
...
`workers/templates/trademark-portfolio-sync/` folder. You need a copy of it on your
...
workers/templates/trademark-portfolio-sync folder"_ — and it'll handle the rest.
...
- **Download a ZIP (no Git needed):** on the cookbook's GitHub page
  ...
  easy to find; the example is inside at `workers/templates/trademark-portfolio-sync/`.
...
`workers/templates/trademark-portfolio-sync/` inside the cookbook you just downloaded.
...
- **In Codex (or another assistant):** ask it to _"Read AGENTS.md and
  DEVELOPMENT.md, then follow `.claude/commands/setup.md` to set up this
  trademark portfolio Worker for me."_
...
[`ARCHITECTURE.md`](ARCHITECTURE.md). The canonical Worker instructions for
your AI assistant live in [`AGENTS.md`](AGENTS.md); recipe-specific design
rules and domain guides live in [`DEVELOPMENT.md`](DEVELOPMENT.md) and
