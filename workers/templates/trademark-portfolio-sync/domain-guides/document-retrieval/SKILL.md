...
3. **TSDR's PDF budget is ~4/min per key**, with multi-minute penalties after
   ...
   bounded in-tool 429 retry only when the complete `Retry-After` delay fits
   inside the shared **55-second operation budget**. Otherwise it returns
   `rate_limited` immediately with `retryAfterSeconds`; it never sleeps past
   the deadline or retries earlier than TSDR requested.
...
match returns `alreadyAttached` with the sub-page id. Missing/repeated cursors
or more results after the five-page safety ceiling fail closed before any PDF
download, upload, or page creation.
