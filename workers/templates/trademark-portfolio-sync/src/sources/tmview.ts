...
const TMVIEW_PAGE_SIZE = 100
const TMVIEW_MAX_PAGES_PER_OWNER = 5
const TMVIEW_MAX_FALLBACK_QUERIES = 20
...
function projectMark(t: AnyRec): ForeignMark | null {
  ...
  return {
    ...
    applicationDate: strictIsoDay(
      t.applicationDate,
      `TMview ${st13} applicationDate`
    ),
    registrationDate: strictIsoDay(
      t.registrationDate,
      `TMview ${st13} registrationDate`
    ),
    expirationDate: strictIsoDay(
      t.expirationDate,
      `TMview ${st13} expirationDate`
    ),
    oppositionDeadline: strictIsoDay(
      t.oppositionDeadLine,
      `TMview ${st13} oppositionDeadLine`
    ), // sic — capital L
...
async function fetchStatusDate(
  ...
  try {
    ...
    const trademark = detail.tradeMark
    if (
      !trademark ||
      typeof trademark !== "object" ||
      Array.isArray(trademark) ||
      !("markCurrentStatusDate" in trademark)
    ) {
      return { ok: false, error: "malformed detail response" }
    }
    const rawStatusDate = rec(trademark).markCurrentStatusDate
    if (
      rawStatusDate === undefined ||
      (typeof rawStatusDate === "string" && rawStatusDate.trim() === "")
    ) {
      return { ok: false, error: "missing markCurrentStatusDate" }
    }
    return {
      ok: true,
      value: strictIsoDay(
        rawStatusDate,
        `TMview detail ${st13} markCurrentStatusDate`
      ),
    }
  } catch (err) {
    return {
...
export async function fetchForeignMarks(
  ...
  let rawHits = 0
  let ownerMatchedHits = 0

  const runSearch = async (
    body: Record<string, unknown>,
    expectedOwner?: RegExp
  ): Promise<{
    raw: number
    matched: number
    signature: string
    ...
    const res = await fetchWithTimeout(
      TMVIEW_SEARCH_URL,
      {
        ...
        body: JSON.stringify({
          page: "1",
          pageSize: String(TMVIEW_PAGE_SIZE),
          criteria: "C",
          ...body,
        }),
      },
    ...
    const payload = rec(await res.json())
    if (!Array.isArray(payload.tradeMarks)) {
      throw new Error("TMview search response has no tradeMarks array")
    }
    const hits = payload.tradeMarks
    let matched = 0
    ...
    for (const h of hits) {
      ...
      const identity = [
        str(hr.ST13),
        str(hr.tmOffice),
        str(hr.applicationNumber),
      ].join("|")
      if (!str(hr.ST13) || !str(hr.tmOffice)) {
        throw new Error("TMview search returned a hit without ST13/office")
      }
      ...
      if (
        applicants.some((applicant) =>
          expectedOwner
            ? expectedOwner.test(applicant)
            : ownerRes.some((re) => re.test(applicant))
        )
      ) {
        matched++
      }
      ...
      if (!m) throw new Error("TMview could not project a search hit")
    ...
    return { raw: hits.length, matched, signature, identities }
  }
  ...
  // that a wordmark text sweep would miss. Pagination is bounded; reaching
  // the bound on a full page fails closed instead of accepting a partial
  // replace-mode data set.
  for (const owner of ownerNames) {
    const expectedOwner = new RegExp(escapeRe(owner), "i")
    const pageSignatures = new Set<string>()
    const seenIdentities = new Set<string>()
    let ownerRawHits = 0
    let ownerMatches = 0
    for (let page = 1; page <= TMVIEW_MAX_PAGES_PER_OWNER; page++) {
      const got = await runSearch(
        { page: String(page), appName: [owner] },
        expectedOwner
      )
      rawHits += got.raw
      ownerMatchedHits += got.matched
      ownerRawHits += got.raw
      ownerMatches += got.matched
      if (got.raw > 0 && pageSignatures.has(got.signature)) {
        throw new Error(
          `TMview repeated a result page while paging applicant "${owner}"; pagination made no progress`
        )
      }
      if (got.identities.some((identity) => seenIdentities.has(identity))) {
        throw new Error(
          `TMview repeated one or more hits while paging applicant "${owner}"; refusing unstable pagination`
        )
      }
      for (const identity of got.identities) seenIdentities.add(identity)
      if (got.raw > 0) pageSignatures.add(got.signature)
      if (got.raw < TMVIEW_PAGE_SIZE) break
      if (page === TMVIEW_MAX_PAGES_PER_OWNER) {
        throw new Error(
          `TMview applicant search for "${owner}" reached the ${TMVIEW_MAX_PAGES_PER_OWNER * TMVIEW_PAGE_SIZE}-hit safety cap; refusing a partial portfolio`
        )
      }
    }
    if (ownerRawHits > 0 && ownerMatches === 0) {
      throw new Error(
        `TMview applicant search returned ${ownerRawHits} raw hits but none matched "${owner}"; the appName filter was likely ignored`
      )
    }
  }
  ...
  if (rawHits === 0 && wordmarks.length > 0) {
    ...
    for (const q of wordmarks.slice(0, TMVIEW_MAX_FALLBACK_QUERIES)) {
      const got = await runSearch({ basicSearch: q })
      rawHits += got.raw
      ownerMatchedHits += got.matched
      if (got.raw === TMVIEW_PAGE_SIZE) {
        throw new Error(
          `TMview fallback search for "${q}" returned a full page; completeness cannot be proven within the bounded fallback budget`
        )
      }
    }
  }
  if (rawHits > 0 && ownerMatchedHits === 0 && ownerNames.length > 0) {
    throw new Error(
      "TMview returned raw hits but none matched a configured owner — refusing an unfiltered or incomplete result"
    )
  }
  ...
  for (const [st13, m] of Object.entries(out)) {
    ...
    if (unchanged && p.statusDate) {
      ...
      detailBudget--
      const fetched = await fetchStatusDate(st13, pace)
      if (fetched.ok) {
        m.statusDate = fetched.value
      } else if (p?.statusDate) {
        console.warn(
          `[tmview] detail ${st13} failed (${fetched.error}) — preserving prior statusDate`
        )
        m.statusDate = p.statusDate
      }
    } else if (p?.statusDate) {
