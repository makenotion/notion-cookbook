# Development guide — Trademark Portfolio Template

[`AGENTS.md`](AGENTS.md) and [`CLAUDE.md`](CLAUDE.md) are symlinks to the
repository's generated, canonical Worker instructions in `.agents/`. This
file is the recipe-specific companion: it documents the trademark portfolio
engine, while `.claude/commands/` preserves guided setup and `domain-guides/`
holds the recipe-specific references. Do not edit generated `.agents/` files in this
template; update the canonical files under `workers/agents/` and run
`npm run agents:sync` from the repository root.
...
If they haven't set the project up yet, Claude Code users can run the
**`/setup`** slash command. Other assistants should follow
`.claude/commands/setup.md`. It is the guided onboarding wizard (owner names,
sources, the optional Docket Inbox, deploy). The base template needs **no API
keys** — never ask the user to register for one to get started; every
credential is an optional upgrade read at run time.
...
5. **Sync state has two size limits.** Saves over 256KB are rejected; worse,
   ...
   an undocumented ceiling that has tightened over time (~200KB in June
   2026; ~99KB wedged production workers in August 2026). Snapshots are
   gzipped; project payloads at the fetch boundary to only fields the join
   reads, and pre-aggregate anything whose count grows without bound
   (invoices!). Every persisted backfill/delta state is measured as serialized
   UTF-8 and rejected at 78,000 bytes before changes are returned. Treat that
   guard as a hard, data-dependent portfolio boundary; never raise it toward
   the observed platform ceiling. (See the `sync-engine` domain guide.)
...
- Row keys: US serial | ST13 |
  `DKT-{office}-{A|R}-{full normalized identifier}`. `A` and `R` separate
  application and registration namespaces; identifiers preserve A–Z/0–9 so
  `AB123` cannot collide with `123`. Legal joins use the same alphanumeric
  normalization and are office-scoped. Office codes are WIPO ST.3-style
  everywhere (`COUNTRY_TO_OFFICE` maps counsel's country names onto them).
...
- **[`source-adapter`](domain-guides/source-adapter/SKILL.md)** — the
  `UsCase` / `ForeignMark` / `OfficialOverlay` contracts and how to write a
  new source. Read before adding an office or register.
- **[`docket-inbox`](domain-guides/docket-inbox/SKILL.md)** — the two report
  formats, the filename convention, every validation guard, and how to adapt
  the parsers to a different firm's export. Read before touching
  `src/sources/counsel-docket.ts`.
- **[`document-retrieval`](domain-guides/document-retrieval/SKILL.md)** —
  how the three tools work (the TSDR file wrapper, the ~4/min PDF budget,
  mark-image validation and upload) and why tools can't use the sync pacers.
  Read before editing `src/tools/`.
